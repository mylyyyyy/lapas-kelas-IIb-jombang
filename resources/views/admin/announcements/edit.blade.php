...
<div class="max-w-4xl mx-auto pb-12 space-y-8 perspective-1000">
    ...
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate__animated animate__fadeInDown">
        <div>
            <h1 class="text-3xl font-extrabold text-gradient">Edit Pengumuman</h1>
            <p class="text-slate-500 mt-1 font-medium">Perbarui informasi pengumuman yang sudah ada.</p>
        </div>
    ...
    <div class="card-3d bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate__animated animate__fadeInUp delay-100">
        ...
        <div class="bg-gradient-to-r from-slate-50 to-blue-50 p-8 border-b border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-500 text-sm">Ubah detail pengumuman di bawah ini.</p>
            </div>
        </div>

        <form action="{{ route('announcements.update', $announcement->id) }}" method="POST" id="editAnnouncementForm" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $announcement->title) }}" 
                        ...
                        placeholder="Contoh: Perubahan Jadwal Kunjungan" required>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select id="status" name="status" class="input-3d w-full px-4 py-3.5 rounded-xl bg-slate-50 text-slate-700 font-medium focus:bg-white appearance-none cursor-pointer" required>
                            <option value="published" @if(old('status', $announcement->status) == 'published') selected @endif>✅ Terbitkan Sekarang</option>
